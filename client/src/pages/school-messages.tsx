...
export default function SchoolMessages() {
  ...
  const sendMutation = useMutation({
    mutationFn: async (data: MessageValues) => {
      const roleLabel = user?.role === "school_admin" ? "Školski administrator" : "Ustanova";
      const body = `[Poruka od ${roleLabel}]\nIme: ${user?.fullName || ""}\nKorisničko ime: ${user?.username || ""}\nUstanova: ${user?.schoolName || "—"}\n\n---\n\n${data.message}`;
      const res = await apiRequest("POST", "/api/contact", {
        name: user?.fullName || user?.username || roleLabel,
        email: `${user?.username || "skola"}@citanje.ba`,
  ...
  const role = (user?.role === "school_admin" ? "school_admin" : "school") as "school_admin" | "school";
    ...
    <DashboardLayout role={role}>
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <p className="text-muted-foreground text-sm">
          Ovdje možete poslati poruku administratorima platforme Čitanje.ba — prijava problema, prijedlozi ili pitanja vezana za vašu ustanovu.
        </p>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="Npr. Pitanje o licencama" data-testid="input-message-subject" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
